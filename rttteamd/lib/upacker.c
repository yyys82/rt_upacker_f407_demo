...
static uint8_t frame_encode(upacker_inst_t packer, uint8_t *data, uint16_t size)
{
    ...
    // 数据头 
    tmp[0] = 0x55;
    tmp[1] = size & 0xff;         //期望发送的数据的个数的低8位
    tmp[2] = (size >> 8) & 0x3f;  //期望发生的数据的个数的高6位 共计14位用于存储数据长度
    crc = tmp[0] ^ tmp[1] ^ tmp[2];//对数据头进行CRC校验
    tmp[2] |= (crc & 0x0C) << 4; //tmp[2][7:6]保存数据头检验[3:2]
    tmp[3] = 0x03 & (crc >> 4);  //tmp[3][1:0]保存数据头校验[5:4]
    for (int i = 0; i < size; i++)
    {
        crc ^= data[i];
    }
    tmp[3] |= (crc & 0xfc);  //tmp[3][7:2]保存data check[7:2]
    packer->send(tmp, 4);    //发送4字节的数据头 
    packer->send(data, size);//发送实际要发送的数据
...
static uint8_t frame_decode(upacker_inst_t packer, uint8_t d)
{
    ...
    {
        packer->state = 1;//收到数据
        packer->calc = 0x55;//CRC校验初值
    }
    else if (packer->state == 1)
    {
        packer->flen = d;//数据长度
        packer->calc ^= d;//crc校验解析
        packer->state = 2;//状态2
    }
    else if (packer->state == 2)
    {
        //长度信息
        packer->flen |= (uint16_t)d << 8;//拼接成完整包含数据个数的flen
        packer->calc ^= d & 0x3F;//crc校验返回
